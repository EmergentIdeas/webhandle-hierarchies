...
import HierarchyView from "./hierarchy-view.mjs"
...
export default function integrate() {
	...
	let tiles = document.querySelector('.ei-editable-sortable-tiles .tiles')
	if (tiles) {
	...
	let hierarchiesTreeHolder = document.querySelector('.hierarchies-tree-holder')
	if (hierarchiesTreeHolder) {
		let nodes = document.querySelector('.hierarchies-fields-form [name="hierarchyFields"]').value
		let view = new HierarchyView({
			nodes: nodes
		})
		view.appendTo(hierarchiesTreeHolder)
		view.render()
		
		let form =  document.querySelector('.hierarchies-fields-form')
		form.addEventListener('submit', (evt) => {
			document.querySelector('.hierarchies-fields-form [name="hierarchyFields"]').value = view.serialize()
		})
